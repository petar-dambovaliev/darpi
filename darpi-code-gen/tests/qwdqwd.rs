...
async fn main() {
    ...
    // unsafe impl Send for RoutePossibilities {}
    // unsafe impl Sync for RoutePossibilities {}
    ...
    pub struct App {
        module: std::sync::Arc<MyModule>,
        handlers: Arc<[RoutePossibilities; 1usize]>,
                ...
                async move {
                    Ok::<_, Infallible>(service_fn(move |r: Request<Body>| {
                        ...
                        async move {
                            ...
                            match handler {
                                RoutePossibilities::a5cb5bfa3b96f21b6eff4509d381261b7 => {
                                    hello_world::call(r, inner_module).await
                                }
                            }
                        }
                    }))
